				...
				<div id="content">
					...
					<div class="searchResult">
						...
						<c:forEach var="product" items="${list}" begin="${pageIndex}"
							...
							<div class="show_pro">
								<li class="pro_list"><a
									href="<%=request.getContextPath()%>/product/DisplayProducts.do?action=getOne_For_Display&pId=${product.p_no}"><div
											class="prod_img">
											<img src="../images/products/${product.p_pic}"
												style="width: 45px;" />
										</div></a></li>
								<li class="pro_list3"><a
									href="<%=request.getContextPath()%>/product/DisplayProducts.do?action=getOne_For_Display&pId=${product.p_no}">${product.p_name}</a><br><span
										style='font-size: 9px'>酒莊:${product.p_winery}<br>國家:${product.p_area}<br>容量:${product.p_vol}ml<br>評分:${product.p_rate}</span></li>
								<li class="pro_list2"><span
									style="color: red; margin-left: 10px; font-size: 26px;">$${product.p_price}</span>
									<br><form action="shoppingCart.do" method="post"><Input type='hidden' name="num" value="1">
													<Input type='hidden' name="no" value="${product.p_no}">
														<Input class="btn btn-success" type="submit"
														style="margin: 0 0 10px 5px;" value='加入購物車'>
												</form></li> <br class="clear" />
							</div>
						</c:forEach>
						...
						<div class="pageSwitch">
							...
							<c:forEach var="page" begin="1" end="${totalPages}">
								<a
									href="DisplayProducts.do?action=${action}&pageNo=${page}${conditionParam}">${page}</a>
							</c:forEach>
							頁
						</div>
					</div>
