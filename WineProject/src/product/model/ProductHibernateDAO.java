...
public class ProductHibernateDAO implements ProductDAO_interface {
	...
	public List<String> findProductType(String productlist) {
		...
		try {
			session.beginTransaction();
			Query query = session.createQuery("SELECT DISTINCT "+productlist+"  FROM ProductVO");
			list1 = query.list();
...
//	-------------------------------------------------------
	public static void main(String arg[]) {
		...
		List<String> list = dao.findProductType("p_style");
...
//		productVOO1.setP_type("white wine");
