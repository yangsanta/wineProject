...
public class Shipping_setDAO implements Shipping_setDAO_interface {
	...
	private static final String UPDATE = "UPDATE Shipping_setVO set min_pirce = :min_pirce";
	...
	public void updateShippingMinPrice(Integer min_pirce) {
		Session session = HibernateUtil.getSessionFactory().getCurrentSession();
		try {
			session.beginTransaction();
			Query query = session.createQuery(UPDATE);
			query.setParameter("min_pirce", min_pirce);
			query.executeUpdate();
			session.getTransaction().commit();
		} catch (RuntimeException ex) {
			session.getTransaction().rollback();
