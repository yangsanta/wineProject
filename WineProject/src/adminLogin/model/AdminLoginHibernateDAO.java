...
public class AdminLoginHibernateDAO  {
	
	private static final String GET_ALL_STMT = "FROM AdminLoginVO order by m_no";
	
	public void insert(AdminLoginVO AdminLoginVO) {
...
//	}
	...
	public AdminLoginVO Login(String id ,String pwd) {
		AdminLoginVO AdminLoginVO = null;
		Session session = HibernateUtil.getSessionFactory().getCurrentSession();
		
		try {
			session.beginTransaction();
			Query query = session.createQuery("FROM AdminLoginVO where a_id=? AND a_pwd=?");
			query.setParameter(0, id);
