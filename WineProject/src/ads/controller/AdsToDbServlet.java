...
public class AdsToDbServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (!errMap.isEmpty()) {
			...
			session.setAttribute("AdsSuccess", productName + " " + fileName
					+ " 廣告設定成功。");
			// request.setAttribute("AdsSuccess", productName + " " +fileName +
			// " 廣告設定成功。");
		}
		if (request.getParameter("action") == null) {
			response.sendRedirect(request.getContextPath()
					+ "/wine_admin/ademin_discount.jsp");
			// request.getRequestDispatcher(request.getContextPath()+"/wine_admin/ademin_discount.jsp").forward(request,
			// response);
		}

		// getAll
		if (request.getParameter("action") != null
				&& request.getParameter("action").equals("getAll")) {
			System.out.println("TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTt"
					+ request.getParameter("action"));
			response.sendRedirect(request.getContextPath()
					+ "/wine_admin/ademin_discount.jsp");
			// request.getRequestDispatcher(request.getContextPath()+"/wine_admin/ademin_discount.jsp?pagetab=2").forward(request,
			// response);
		}
